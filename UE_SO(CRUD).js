...
define(['N/record', 'N/https', 'N/log', 'N/search', 'N/file'], function(record, https, log, search, file) {
    ...
    function findSalesforceAccount(companyName, config) {
        var headers = {
        ...
        var queryUrl = config.endpointUrl + '/services/data/v60.0/query/?q=' +
            encodeURIComponent('SELECT Id FROM Account WHERE Name = \'' + companyName + '\'');
    ...
    function syncCustomerToSalesforceAsAccount(customerId, config) {
        ...
        var sfAccountId = findSalesforceAccount(companyName, config);
        if (sfAccountId) {
            log.debug('Account already exists in Salesforce', sfAccountId);
            return sfAccountId;
        }
    ...
    function createSalesforceOrder(salesOrderRecord, sfAccountId, itemMappings, config) {
        var orderData = {
            ...
            "celigo_sfnsio_Total_NetSuite__c": salesOrderRecord.getValue('total'),
            "celigo_sfnsio_Tax_Total_NetSuite__c": salesOrderRecord.getValue('taxtotal')
        };
    ...
    function getSalesforceOrderItems(sfOrderId, config) {
        var headers = {
            'Content-Type': 'application/json',
            'Authorization': 'Bearer ' + config.refreshToken
        };
        ...
        var queryUrl = config.endpointUrl + '/services/data/v60.0/query/?q=' +
            encodeURIComponent('SELECT Id, PricebookEntryId, Quantity FROM OrderItem WHERE OrderId = \'' + sfOrderId + '\'');
        ...
        if (response.code === 200) {
            ...
            log.error('Failed to retrieve Salesforce Order Items', response.body);
            return [];
        }
    }

    function removeSalesforceOrderItem(orderItemId, config) {
        var headers = {
            'Content-Type': 'application/json',
            'Authorization': 'Bearer ' + config.refreshToken,
        ...
        var response = https.delete({
            url: config.endpointUrl + '/services/data/v60.0/sobjects/OrderItem/' + orderItemId,
            headers: headers
        });

        if (response.code === 204) {
            log.debug('Salesforce OrderItem Deletion Successful', 'OrderItem ID: ' + orderItemId);
        } else {
            log.error('Salesforce OrderItem Deletion Failed', response.body);
        }
    }

    function updateSalesforceOrder(salesOrderRecord, sfOrderId, config) {
        var orderData = {
            "celigo_sfnsio_Total_NetSuite__c": salesOrderRecord.getValue('total'),
            "celigo_sfnsio_Tax_Total_NetSuite__c": salesOrderRecord.getValue('taxtotal')
        };
        ...
        var response = https.post({
            url: config.endpointUrl + '/services/data/v60.0/sobjects/Order/' + sfOrderId +'?_HttpMethod=PATCH',
            headers: headers,
            body: JSON.stringify(orderData)
        });

        if (response.code === 200 || response.code === 204) {
            log.debug('Salesforce Order Update Successful', 'Salesforce Order ID: ' + sfOrderId);

            
            var existingSalesforceItems = getSalesforceOrderItems(sfOrderId, config);

           
            var itemMappings = syncItemsToSalesforceAsProducts(salesOrderRecord, config);

            
            var currentNetSuiteItemIds = Object.keys(itemMappings);
            existingSalesforceItems.forEach(function(orderItem) {
                var isItemStillInNetSuiteOrder = currentNetSuiteItemIds.some(function(netSuiteItemId) {
                    return itemMappings[netSuiteItemId].priceBookEntryId === orderItem.PricebookEntryId;
                });

                if (!isItemStillInNetSuiteOrder) {
                    removeSalesforceOrderItem(orderItem.Id, config);
                }
            });

           
            currentNetSuiteItemIds.forEach(function(itemId) {
                var mapping = itemMappings[itemId];
                if (mapping && mapping.priceBookEntryId) {
                    var orderItemData = {
                        "OrderId": sfOrderId,
            ...
            log.error('Salesforce Order Update Failed', response.body);
        }
    }
        ...
        if (context.type === context.UserEventType.CREATE) {
            ...
            if (sfAccountId) {
                ...
                record.submitFields({
                    ...
                    values: {
                        custbody_salesforce_account_id: sfAccountId,
                        custbody_salesforce_order_id: sfOrderId
                    }
                });
                log.debug('Salesforce Order Creation Successful', 'Salesforce Order ID: ' + sfOrderId);
            }
        } else if (context.type === context.UserEventType.EDIT) {
            var sfOrderId = salesOrderRecord.getValue('custbody_salesforce_order_id');
            if (sfOrderId) {
                updateSalesforceOrder(salesOrderRecord, sfOrderId, config);
            } else {
                log.error('Salesforce Order ID not found on NetSuite Sales Order');
            }
        }
    }
